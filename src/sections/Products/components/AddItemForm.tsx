...
const emptyForm: AddItemFormData = {
  title: '', use_competitor_title: false, competitor_link: '', date: '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const product = await ProductModel.create({
        organization_id: activeOrg.id, created_by: user.id,
        title: form.title.trim(), status: form.status as ProductStatus,
        date: form.date || null, photo_url: null,
      ...
      if (form.photo) {
        const photoUrl = await ProductModel.uploadPhoto(form.photo, activeOrg.id, product.id);
    ...
    <div className="animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <section className="card p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <div className="sm:col-span-2 lg:col-span-3">
              ...
              <Input id="item-title" placeholder="Product title" value={form.title} onChange={(e) => { set('title', e.target.value); if (fieldErrors.title) setFieldErrors((p) => ({ ...p, title: undefined })); }} className={fieldErrors.title ? 'border-red-500 focus:border-red-500' : ''} />
              ...
              <div className="flex items-center justify-between mt-3 px-3.5 py-2.5 bg-ds-surface2 border border-ds-border rounded-xl">
                ...
                <Switch id="item-use-competitor-title" checked={form.use_competitor_title} onCheckedChange={(val) => set('use_competitor_title', val)} />
              </div>
            </div>
            <div>
              <Label htmlFor="item-date">Date</Label>
              <Input id="item-date" type="date" value={form.date} onChange={(e) => set('date', e.target.value)} />
            </div>
            <div>
              <Label htmlFor="item-status">Status</Label>
              <select id="item-status" className={selectClass} value={form.status} onChange={(e) => set('status', e.target.value)}>
            ...
            <div>
              ...
              <select id="item-gender" className={selectClass} value={form.gender} onChange={(e) => set('gender', e.target.value)}>
            ...
            <div>
              ...
              <select id="item-season" className={selectClass} value={form.season} onChange={(e) => set('season', e.target.value)}>
        ...
        <section className="card p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <div>
              ...
              <TagInput id="item-colors" placeholder="e.g. Red" tags={form.colors} onAdd={(v) => addTag('colors', v)} onRemove={(i) => removeTag('colors', i)} />
            </div>
            <div>
              <Label htmlFor="item-sizes">Sizes</Label>
              <TagInput id="item-sizes" placeholder="e.g. M" tags={form.sizes} onAdd={(v) => addTag('sizes', v)} onRemove={(i) => removeTag('sizes', i)} />
            </div>
            <div>
              <Label htmlFor="item-material">Material</Label>
              <Input id="item-material" placeholder="e.g. 100% Cotton" value={form.material} onChange={(e) => set('material', e.target.value)} />
            </div>
          </div>
        </section>
        ...
        <section className="card p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            <div>
              <Label htmlFor="item-purchase-price">Purchase Price <span className="text-red-400">*</span></Label>
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-ds-muted text-sm">$</span>
                <Input id="item-purchase-price" type="number" min="0" step="0.01" placeholder="0.00" className={`pl-7${fieldErrors.purchase_price ? ' border-red-500 focus:border-red-500' : ''}`} value={form.purchase_price} onChange={(e) => { set('purchase_price', e.target.value); if (fieldErrors.purchase_price) setFieldErrors((p) => ({ ...p, purchase_price: undefined })); }} />
              </div>
            ...
            <div>
              ...
              <select id="item-base-currency" className={selectClass} value={form.base_currency}
                onChange={(e) => { const next = e.target.value; setForm((prev) => ({ ...prev, base_currency: next, converted_currency: prev.converted_currency ? prev.converted_currency : '' })); }}>
              ...
              <div className="mt-3 flex items-end gap-2">
                <div className="flex-1">
                  ...
                  <select id="item-converted-currency" className={selectClass} value={form.converted_currency} onChange={(e) => set('converted_currency', e.target.value)}>
        ...
        <section className="card p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              <Label htmlFor="item-competitor-link">Competitor Link <span className="text-red-400">*</span></Label>
              <Input id="item-competitor-link" type="url" placeholder="https://" value={form.competitor_link} onChange={(e) => { set('competitor_link', e.target.value); if (fieldErrors.competitor_link) setFieldErrors((p) => ({ ...p, competitor_link: undefined })); }} className={fieldErrors.competitor_link ? 'border-red-500 focus:border-red-500' : ''} />
            ...
            <div>
              ...
              <Input id="item-supplier-link" type="url" placeholder="https://" value={form.supplier_link} onChange={(e) => set('supplier_link', e.target.value)} />
            </div>
          </div>
        </section>
        ...
        <section className="card p-6">
          ...
          <Textarea id="item-note" placeholder="Add any additional notes about this product…" rows={4} value={form.note} onChange={(e) => set('note', e.target.value)} />
        </section>
        ...
        <section className="card p-6">
          <div className="flex items-center justify-between gap-4">
            ...
            <Switch id="add-to-optimize" checked={form.to_optimize} onCheckedChange={(val) => set('to_optimize', val)} />
          </div>
        </section>

        <div className="flex items-center justify-end gap-3 pb-8">
          <Button type="button" variant="secondary" onClick={handleReset} disabled={saveState === 'saving'}>Reset</Button>
          <Button type="submit" variant="primary" disabled={saveState === 'saving'}>
            {saveState === 'saving' ? <><Loader2 size={14} className="animate-spin" />Saving…</> : 'Save Item'}
          </Button>
        </div>
      </form>
    </div>
