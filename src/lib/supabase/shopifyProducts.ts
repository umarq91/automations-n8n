...
export async function getShopifyProducts(organizationId: string): Promise<ShopifyProduct[]> {
...
export async function getShopifyConnection(organizationId: string): Promise<ShopifyConnection> {
  const { data, error } = await supabase
    .from('integrations')
...
export async function syncShopifyProducts(organizationId: string): Promise<SyncResult> {
  const { data, error } = await supabase.functions.invoke('shopify-sync-products', {
    body: { org_id: organizationId },
  });

  if (error) {
    return { ok: false, message: error.message ?? 'Failed to sync Shopify products.' };
  }
